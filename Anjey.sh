...
echo -e "$GREEN IP = $RED `ifconfig enp0s3 | awk -grep '{print $2}'` $RED \n"
echo -e "$GREEN Lost 10 secure =$END `tail -10 /var/log/messages` $END \n"
echo -e "$GREEN Active user = $RED `w` $END \n"
echo -e "$GREEN Uptime = $RED `uptime | awk'{pirint$3}'` $END \n"
